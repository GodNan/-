...
public class SM4 {
    ...
    private static final String SM4_KEY = "4jgj27koas8957423iof90875cjh289jnDSd89hr98eu98e1dWbdpf8789info2596xxzx20169JSKd48y8yggf9al952131ovt5uqea4s5dw7841fl3k4f8d7bv93td";
    ...
    public static byte[] encodeSM4(String plaintext, byte[] key) {
        ...
        return SM4.encodeSM4(plaintext.getBytes(Charset.forName("UTF-8")), key);
    }
     ...
     * @return
     */
    public static byte[] encodeSM4(byte[] plaintext, byte[] key) {
    ...
    public static byte[] encodeSM4WithKey(byte[] plaintext) {
        return encodeSM4(plaintext, SM4_KEY.getBytes());
    }
     ...
     * @return
     */
    public static byte[] decodeSM4(byte[] ciphertext, byte[] key) {
    ...
    public static byte[] decodeSM4WithKey(byte[] ciphertext) {
        return decodeSM4(ciphertext, SM4_KEY.getBytes());
    }
     ...
     */
    public static String decodeSM4toString(byte[] ciphertext, byte[] key) throws UnsupportedEncodingException {
        byte[] plaintext = new byte[ciphertext.length];
        plaintext = decodeSM4(ciphertext, key);
    ...
    public static String getStringAfterEncrypted(String nameForEncrypt){
    	if(nameForEncrypt.length() < 24){
    		byte[] enOut = encodeSM4(nameForEncrypt, SM4_KEY.getBytes());
    		return Base64Utils.encode(enOut);
    	}
    	return nameForEncrypt;
    }
    
    public static String getStringAfterDecrypted(String stringAfterEncrypted) throws UnsupportedEncodingException {
    	if(stringAfterEncrypted.length() < 24){
    		return stringAfterEncrypted;
    	}
    	return decodeSM4toString(Base64Utils.decode(stringAfterEncrypted), SM4_KEY.getBytes());
    }
